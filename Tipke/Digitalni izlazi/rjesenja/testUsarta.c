...
void read_keypad(uint8_t device_addr) {
	...
	if (pressed_keys != 0) {
		...
		// Prikaži koje tipke su pritisnute
...
// Pojednostavljeno skeniranje samo jednog ure?aja
void scan_single_keypad(uint8_t device_addr) {
	...
	// Detektuj novo pritiskanje (edge detection)
...
int main(void) {
	...
	// Inicijaliziraj sve MCP23008 za ?itanje tipki
	...
	// Glavna petlja - kontinuirano skeniranje
	while (1) {
		// Opcija 1: Skeniraj sve ure?aje
		scan_all_keypads();
		
		// Opcija 2: Skeniraj samo jedan ure?aj (zakomentiraj gornju liniju)
		// scan_single_keypad(MCP23008_ADDR1);
		
		_delay_ms(50); // 50ms interval skeniranja
	}
